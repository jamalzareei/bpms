...
class PisController extends Controller
{
    ...
    public function addPiPost(Request $request)
    {
        ...
        if (!($customer && $customer->code && $customer->factory && $customer->factory->code && $customer->country && $customer->country->code)) {
        ...
        $code = $customer->country->code . '-' . $customer->code . '-' . $customer->factory->code . '-' . date("dm", strtotime($request->date)) . '-' . $request->extra_code;
